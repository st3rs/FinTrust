...
FinTrust today is a frontend dashboard that *displays* invoices but has no real
document-generation backend. We are adding an API-first **docgen spine** so invoices
become real, archived, downloadable PDFs.
...
**Phase 1 — docgen spine (current focus)**
Template engine + Gotenberg PDF rendering + Supabase Storage + per-account API keys.
A default Thai-friendly invoice template (VAT 7%, WHT, PromptPay QR slot).
OUT of scope for Phase 1: RBAC roles, team members, Pro feature gating,
watermarking, recurring invoices.
...
> the `scripts` block in `package.json` before relying on them. Commands marked
> *(planned)* do not exist yet; they are the intended interface for Phase 1 and
> should be created as part of that work.
...
### docgen service — Phase 1 *(planned, services/docgen/)*
...
- docgen *(planned)*: `GOTENBERG_URL`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`,
  `SUPABASE_STORAGE_BUCKET`, `DOCGEN_API_KEYS_SALT`.
